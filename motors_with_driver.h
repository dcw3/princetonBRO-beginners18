...
int maxPower = 0;
const int LUDICROUS_SPEED = 255; // Max power, to fast for normal movement.
...
const int scaleA = 1; // Motors not same speed? Change these to scale them.
const int scaleB = 1;
...
void setMotor(int direction, double power) {
    switch (direction) {
        case NORTH : northMove(power); break;
        case SOUTH : northMove(power * -1); break;
        case EAST :  eastMove(power); break;
        case WEST : westMove(power * -1); break;
    }
}
